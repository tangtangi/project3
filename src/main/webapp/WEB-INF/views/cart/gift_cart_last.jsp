		...
		<c:forEach var="cartgift" items="${cartgift }" varStatus="stt">
			<div class="littleContent">
				<div class="little" style="width: 50%; height: 90px;">
					...
					<div class="content" >
						<p style="margin-left: 15px;margin-bottom: 8px;  margin-top: 17px;">[모바일]${cartgift.pizza}</p>
<%-- 						<p style="text-align: left;margin-left: 70px;"><fmt:formatNumber type="number" maxFractionDigits="3" value="${cartgift.price_L}" />원</p> --%>
				...
				<div class="little" style="width: 30%;justify-content: center;"><fmt:formatNumber type="number" maxFractionDigits="3" value="${cartgift.price_L * cartgift.count }" />원</div>
			</div>
		</c:forEach>  
		<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -->
		...
		<div class="form_box" style="clear: both;" >
			...
			<c:set var="total_price" value="${sum - discount}"/>
			<table style="margin-left: 80px;margin-top: 55px;margin-bottom: 55px;">
				<tr>
					<td class="paybox" id="kakao"><a href="kakao_gift?total_price=${total_price }">카카오페이 결제</a></td>
					<td width=10px></td>
<%-- 					<td class="paybox" id="mutong"><a href="bank?total_price=${total_price }">무통장 결제</a></td> --%>
				</tr>
			</table>
		...
		<div style="display: flex;justify-content: center; padding: 40px 40px 40px 0; line-height: 4;">
			...
			<a href="#" class="goToGift" style="color: white;background: #ff143c; text-align: center;" onclick="orderButton()">결제하기</a>
		</div>	
	</div><!-- 중간폭 -->
	<div style ="width: 13.5%"></div>
</div>
